<#
 -----------------------------------------------------------------------------
 Batch script for launching TcUnit-Runner
 For documentation see: www.tcunit.org
 -----------------------------------------------------------------------------
 Instructions:
 Set the below variable TCUNIT_RUNNER_INSTALL_DIRECTORY to where TcUnit-Runner was
 installed. This should normally be left at default and not changed
 (default C:\Program Files (x86)\TcUnit-Runner)
 ...
 -a [OPTIONAL] The AMS NetId of the device of where the project and TcUnit should run.
    If this is not provided, the local AMS NetId is assumed (127.0.0.1.1.1)
 -w [OPTIONAL] The version of TwinCAT to be used. If this is not provided, then the following rules apply:
    - If TwinCAT project is pinned, go with this version, otherwise...
    - Go with latest installed version of TwinCA -u [OPTIONAL] Timeout the process with an error after X minutes. If no timeout is provided,
               the process might run indefinitely in case of error #>

$TCUNIT_RUNNER_INSTALL_DIRECTORY=C:\Users\JEB\Source\Repos\TcUnit-Runner_Bulkje\TcUnit-Runner\bin\Debug
$TCUNIT_TASK_NAME=
$TCUNIT_AMSNETID=
$TCUNIT_TCVERSION_TO_USE=
$TCUNIT_TIMEOUT=
...
$TCUNIT_RUNNER_EXECUTABLE_COMPLETE_PATH=$TCUNIT_RUNNER_INSTALL_DIRECTORY.\TcUnit-Runner.exe

# Check that the TcUnit-Runner executable exists
...
# Error handling of finding the files.
...
# %errorlevel% is a system wide environment variable that is set upon execution of a program
echo Exit code is %errorlevel%

EXIT /B %errorlevel%

# This function process the parameters 
:Process_Parameters
